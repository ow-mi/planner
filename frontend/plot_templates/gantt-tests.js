...
controls.append('label')
  ...
  .html(`
    ...
    <select id="colorModeSelect" style="font-size:12px; padding:2px 6px;">
      ...
      <option value="fte_group">FTE Group</option>
...
controls.append('label')
  ...
  .style('align-items', 'center')
  .style('gap', '6px')
  .html(`<input id="legendToggle" type="checkbox" /> Show legend`);
...
controls.append('label')
  ...
  .style('align-items', 'center')
  .style('gap', '6px')
  .html(`<input id="labelsToggle" type="checkbox" checked /> Show labels`);

// Process and parse data - handle both snake_case and camelCase field names
const processed = testSchedules.map((d, index) => {
  ...
  const equipmentLabel =
    firstAssigned(d.assigned_equipment ?? d.assignedEquipment) ||
    firstAssigned(d.assigned_equipment_names ?? d.assignedEquipmentNames) ||
    firstAssigned(d.assigned_equipment_id ?? d.assignedEquipmentId) ||
    '';
  const fteLabel =
    firstAssigned(d.assigned_fte ?? d.assignedFte) ||
    firstAssigned(d.assigned_fte_names ?? d.assignedFteNames) ||
    firstAssigned(d.assigned_fte_id ?? d.assignedFteId) ||
    '';
  const equipmentGroupLabel =
    firstAssigned(d.assigned_equipment_groups ?? d.assignedEquipmentGroups) ||
    firstAssigned(d.assigned_equipment_group ?? d.assignedEquipmentGroup) ||
    '';
  const fteGroupLabel =
    firstAssigned(d.assigned_fte_groups ?? d.assignedFteGroups) ||
    firstAssigned(d.assigned_fte_group ?? d.assignedFteGroup) ||
    '';

  return {
    id: `${d.project_leg_id ?? d.projectLegId ?? 'unknown'}-${d.test_name ?? d.testName ?? d.test_id ?? d.testId}-${start.getTime()}-${end.getTime()}-${index}`,
    ...
    fte: fteLabel,
    equipment_group: equipmentGroupLabel,
    fte_group: fteGroupLabel,
    test_short: (d.test_name ?? d.testName ?? d.test_id ?? d.testId ?? 'Test'),
    isHolidayBuffer: Boolean(d.is_holiday_buffer ?? d.isHolidayBuffer ?? false)
  };
});

console.info('[vis-name-debug] Gantt input assignment sample', testSchedules.slice(0, 5).map((d, index) => ({
  index,
  testId: d.test_id ?? d.testId,
  projectLegId: d.project_leg_id ?? d.projectLegId,
  rawAssignedEquipment: d.assigned_equipment ?? d.assignedEquipment ?? d.assigned_equipment_id ?? d.assignedEquipmentId ?? '',
  rawAssignedFte: d.assigned_fte ?? d.assignedFte ?? d.assigned_fte_id ?? d.assignedFteId ?? '',
  rawAssignedEquipmentGroup: d.assigned_equipment_group ?? d.assignedEquipmentGroup ?? d.assigned_equipment_groups ?? d.assignedEquipmentGroups ?? '',
  rawAssignedFteGroup: d.assigned_fte_group ?? d.assignedFteGroup ?? d.assigned_fte_groups ?? d.assignedFteGroups ?? ''
})));
console.info('[vis-name-debug] Gantt processed labels sample', processed.slice(0, 5).map((d, index) => ({
  index,
  id: d.id,
  leg: d.leg,
  equipment: d.equipment,
  fte: d.fte,
  equipmentGroup: d.equipment_group,
  fteGroup: d.fte_group
})));
...
const dateColsReserve = 210; // Start/End/Duration columns

let legendVisible = false; // must be declared before rightReserve() which closes over it
...
const endColX = () => width + 70;
const durationColX = () => width + 135;
...
let colorMode = 'equipment'; // 'equipment' | 'fte' | 'equipment_group' | 'fte_group' | 'leg'
...
const categoryValue = (d) => {
  ...
  if (colorMode === 'equipment_group') return (d.equipment_group && String(d.equipment_group).trim()) ? d.equipment_group : NONE_LABEL;
  if (colorMode === 'fte_group') return (d.fte_group && String(d.fte_group).trim()) ? d.fte_group : NONE_LABEL;
...
const isBufferTest = (d) => {
  if (!d) return false;
  if (d.isHolidayBuffer) return true;
  return String(d.test || '').trim().toLowerCase() === 'buff';
};

const testsByLegChronological = new Map();
processed.forEach((item) => {
  const legId = String(item?.leg || '').trim();
  if (!legId) return;
  if (!testsByLegChronological.has(legId)) {
    testsByLegChronological.set(legId, []);
  }
  testsByLegChronological.get(legId).push(item);
});
testsByLegChronological.forEach((items) => {
  items.sort((a, b) => {
...
const resolvedCategoryValue = (d) => {
  if (!isBufferTest(d)) {
    return categoryValue(d);
  }

  const legId = String(d?.leg || '').trim();
  const legItems = testsByLegChronological.get(legId) || [];
  const currentIndex = legItems.indexOf(d);
  if (currentIndex <= 0) {
    return categoryValue(d);
  }

  for (let i = currentIndex - 1; i >= 0; i -= 1) {
    const previousItem = legItems[i];
    if (!isBufferTest(previousItem)) {
      return categoryValue(previousItem);
    }
  }
...
const categoryLabel = () => {
  ...
  if (colorMode === 'equipment_group') return 'Equipment Group:';
  if (colorMode === 'fte_group') return 'FTE Group:';
  if (colorMode === 'leg') return 'Leg:';
  return 'Equipment:';
};

const categoriesForMode = () => {
  const vals = processed.map(resolvedCategoryValue);
  return [...new Set(vals)];
};
...
d3.select(container).select('#labelsToggle').on('change', function() {
...
const resizeChart = () => {
  ...
  legDateCols.select('.date-header-duration').attr('x', durationColX());
  legDateCols.selectAll('.leg-start-label').attr('x', startColX());
  legDateCols.selectAll('.leg-end-label').attr('x', endColX());
  legDateCols.selectAll('.leg-duration-label').attr('x', durationColX());
...
const updateBars = () => {
  const filteredData = processed.filter(d => activeCategories.has(resolvedCategoryValue(d)));
  ...
  const bars = barsGroup.selectAll('rect.bar')
    ...
    .attr('fill', d => {
      const cat = resolvedCategoryValue(d);
  ...
  bars.append('title')
    .text(d => {
      ...
      return [
        ...
        `FTE: ${d.fte || 'N/A'}`,
        `Equipment Group: ${d.equipment_group || 'N/A'}`,
        `FTE Group: ${d.fte_group || 'N/A'}`
      ].join('\n');
    });
};
...
const updateLabels = () => {
  const filteredData = labelsVisible
    ? processed.filter(d => activeCategories.has(resolvedCategoryValue(d)))
    : [];
...
legDateCols.append('text')
  .attr('class', 'date-header-duration')
  .attr('x', durationColX())
  .attr('y', -25)
  .attr('text-anchor', 'start')
  .style('font-size', '11px')
...
legs.forEach(leg => {
  ...
  const weeks = Math.max(0, (e.getTime() - s.getTime()) / (7 * 24 * 60 * 60 * 1000));
  const weeksRounded = Math.round(weeks * 10) / 10;
  legDateCols.append('text')
    .attr('class', 'leg-duration-label')
    .attr('x', durationColX())
    .attr('y', legPositions[leg] + 5)
    .attr('text-anchor', 'start')
    .attr('alignment-baseline', 'middle')

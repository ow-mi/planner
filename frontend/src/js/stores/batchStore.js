...
// Helper functions for deadline/scenario validation
function normalizeScenario(scenario) {
    return {
        name: String(scenario?.name || '').trim(),
        timeLimit: Number.isFinite(Number(scenario?.timeLimit)) ? Number(scenario.timeLimit) : 300,
        debugLevel: scenario?.debugLevel || 'INFO',
        outputFolder: scenario?.outputFolder ? String(scenario.outputFolder).trim() : null
    };
}
...
function validateScenarios(scenarios) {
    ...
    scenarios.forEach((scenario, index) => {
        ...
        const timeLimit = Number(scenario?.timeLimit);
...
document.addEventListener('alpine:init', () => {
    Alpine.store('batch', {
        ...
        init() {
            try {
                if (this.isInitialized) {
                    console.log('[batchStore] Already initialized, skipping');
                    return;
                }
                ...
                this.error = 'Failed to initialize batch storage';
            }
        },
        ...
        validateScenarios() {
            return validateScenarios(this.scenarios);
        },
        ...
        setInputsFromFileStore() {
            try {
                ...
                const fileMap = typeof filesStore.getSolverInputData === 'function'
                    ? filesStore.getSolverInputData()
                    : {};
                this.inputs = Object.entries(fileMap).map(([name, content]) => ({ name, content }));
                this.folderPath = typeof filesStore.getCurrentFolderPath === 'function'
        ...
        getComparisonRows() {
            ...
            const indexedScenarios = this.scenarios.map((scenario, index) => ({
                ...
                idKey: this.normalizeLookupKey(scenario?.id || scenario?.scenarioId)
            }));
        ...
        findBestResultMatchIndex(resultItems, scenarioEntry, usedResultIndexes) {
            ...
            if (scenarioEntry.idKey) {
                index = resultItems.findIndex((item, itemIndex) => {
                    ...
                    const itemIdKey = this.normalizeLookupKey(item?.scenarioId || item?.id);
            ...
            if (scenarioEntry.nameKey) {
                index = resultItems.findIndex((item, itemIndex) => {
                    ...
                    const itemNameKey = this.normalizeLookupKey(item?.scenarioName || item?.name || item?.scenario);
        ...
        createComparisonRow(resultItem, scenario, index, hasResult) {
            ...
            const scenarioLabel = String(
                resultItem?.scenarioName
                || resultItem?.name
                || resultItem?.scenario
                || scenario?.name
                || fallbackScenarioLabel
            ).trim();
            const scenarioId = String(resultItem?.scenarioId || resultItem?.id || scenario?.scenarioId || scenario?.id || '').trim() || 'N/A';
            ...
            const objectiveValue = this.pickValue(resultItem, [
                ...
                ['solverStats', 'objectiveValue']
            ]);
            const solveTimeValue = this.pickValue(resultItem, [
                ['metrics', 'solveTime'],
                ['metrics', 'solveTimeSeconds'],
                ['kpis', 'solveTime'],
                ['kpis', 'solveTimeSeconds'],
                ['solverStats', 'solveTime'],
                ['solveTime'],
                ['solveTimeSeconds']
            ]);
        ...
        normalizeStatusValue(resultItem, hasResult) {
            ...
            const status = String(resultItem?.status || resultItem?.state || resultItem?.resultStatus || '').trim();
            if (!status) {
                return 'UNKNOWN';
            }
        ...
        extractArtifactReferences(resultItem) {
            ...
            if (resultItem.outputFiles && typeof resultItem.outputFiles === 'object') {
                const outputEntries = Object.keys(resultItem.outputFiles).map((filename) => ({
                    label: filename,
                    value: 'Available'
                }));
        ...
        async runBatch() {
            ...
            try {
                ...
                this.sessionId = sessionResponse.session_id || sessionResponse.sessionId || sessionResponse.id || null;
                ...
                if (!this.folderPath) {
                    throw new Error('Folder path is required. Import a folder in Input Data first.');
                }

                this.message = 'Importing folder inputs...';
                await this.apiService.importSessionInputsFromFolder(this.sessionId, this.folderPath);
                ...
                this.batchId = submitResponse.batch_id || submitResponse.batchId || submitResponse.id || null;
                if (!this.batchId) {
...
// Support both browser and Node.js (testing) environments

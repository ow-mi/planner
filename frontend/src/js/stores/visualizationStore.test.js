...
describe('visualizer component csvData guards', () => {
    ...
    beforeAll(() => {
        const visualizerPath = path.join(process.cwd(), 'frontend', 'src', 'components', 'visualizer.html');
        componentHtml = fs.readFileSync(visualizerPath, 'utf8');
        const scriptMatch = componentHtml.match(/<script>([\s\S]*?)<\/script>/);
        const storePath = path.join(process.cwd(), 'frontend', 'src', 'js', 'stores', 'visualizationStore.js');

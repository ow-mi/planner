/**
 * FTE Utilization Gantt Chart Module
 *
 * This module creates a D3 Gantt chart for displaying FTE (Full-Time Equivalent)
 * resource usage over time.
 *
 * Data Source: output/data/fte_usage.csv
 * Expected columns: fte_id, test_id, test_name, start, end
 *
...
function createD3GanttFTE(raw) {
  ...
  const processed = raw.map((d) => {
    ...
    if (d.start_date && d.start_time) {
      ...
      console.warn('No start date/time found for FTE test:', d.test_id);
    ...
    if (d.end_date && d.end_time) {
      ...
      console.warn('No end date/time found for FTE test:', d.test_id);
  ...
  content.selectAll('rect.bar')
    ...
    .attr('y', (d) => yScale(d.fte) + yScale.bandwidth() * 0.2)

    ...
    if column_types and not df.empty:
        for col, expected_type in column_types.items():
            ...
            if expected_type == int:
                ...
                type_ok = pd.api.types.is_float_dtype(
                    actual_type
                ) or pd.api.types.is_integer_dtype(actual_type)
            elif expected_type == str:
                type_ok = pd.api.types.is_string_dtype(
                    actual_type
...
def validate_input_files(input_folder: str) -> Tuple[bool, List[str]]:
    ...
    csv_files = {
        ...
        "data_equipment.csv": {
            "columns": [
                "equipment_id",
                "available_start_week_iso",
                "available_end_week_iso",
            ],
            "types": {},
        },
        "data_test_duts.csv": {
...
def validate_iso_week_format(iso_week: str) -> bool:
    """Validate week format for legacy and canonical inputs."""
    if iso_week is None:
        return False

    value = str(iso_week).strip()
    ...
    return 1900 <= year <= 2100 and 1 <= week <= 53 and 0 <= fraction <= 6

...
describe('dataEditor row operation persistence', () => {
     ...
     beforeAll(() => {
         ...
         const dataEditorPath = path.join(process.cwd(), 'ui_v2_exp', 'src', 'components', 'data-editor.html');
...
describe('fileUpload template key stability', () => {
     test('file upload list should use deterministic unique keys', () => {
          const fs = require('fs');
          const path = require('path');
          const fileUploadPath = path.join(process.cwd(), 'ui_v2_exp', 'src', 'components', 'file-upload.html');
...
describe('configuration checkbox toggling', () => {
    test('disabled sections should not block checkbox interaction', () => {
        const fs = require('fs');
        const path = require('path');
        const cssPath = path.join(process.cwd(), 'ui_v2_exp', 'src', 'styles', 'base.css');

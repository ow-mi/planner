...
const TAB_CONFIG = {
    ...
    'configuration': { url: 'components/config-editor.html', target: '#config-editor-container' },
    'solver': { url: 'components/solver-controls.html', target: '#solver-controls-container' },
    'batch': { url: 'components/batch-editor.html', target: '#batch-editor-container' },
    'visualizer': { url: 'components/visualizer.html', target: '#visualizer-container' },
    'output_data': { url: 'components/output-viewer.html', target: '#output-viewer-container' }
};

// Tab target mapping for visibility updates
const TAB_TARGETS = {
    'input_data': '#file-upload-container',
    'edit_data': '#data-editor-container',
    'configuration': '#config-editor-container',
    'solver': '#solver-controls-container',
    'batch': '#batch-editor-container',
 ...
 * @param {string} hash - Window location hash (e.g., '#input_data')
 * @returns {string} Tab name or 'input_data' as default
 */
function getTabFromHash(hash) {
    const hashTab = (hash || window.location.hash).replace('#', '').trim();
    return VALID_TABS.includes(hashTab) ? hashTab : 'input_data';
}
